...
from eql.utils import get_indices, get_una_bin_funs
...
f_dict_sy = {
    ...
    custom_functions.div_una: lambda x: 1/(x+1),
    jnp.exp: sy.exp,
...
def get_symbolic_expr(params, functions, use_l0=False):
    ...
    for i in range(len(pdict)-1):
        hidden.append(pdict[f'layers_{i}'])
...
def get_symbolic_expr_div(params, functions):
    ...
    a = get_symbolic_expr_layer(
        *get_Wb(hidden[0]["linear_layer"]), functions[0]
    )

    # skip first hidden layer
    for i in range(1, len(hidden)):
        b = get_symbolic_expr_layer(
            *get_Wb(hidden[i]["linear_layer"]), functions[i], var_name="b"
        )
        c = []
        for k in range(len(b)):
            c.append(b[k].subs({"b" + str(j): a[j] for j in range(len(a))}))
    ...
    # get weight/bias of last (div) linear layer
    w, b = last["kernel"], last["bias"]
    z = []
    ...
    for i in range(out_features//2):
        out.append(sy.Symbol.__truediv__(z[i], z[i+1]))
